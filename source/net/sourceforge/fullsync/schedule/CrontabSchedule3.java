...
public class CrontabSchedule3 implements Schedule {
	public static String SCHEDULE_TYPE = "crontab3";
	...
	public static CrontabSchedule3 unserialize(final Element element) throws DataParseException {
		String pattern = "* * * * *";
		if (element.hasAttribute("pattern")) {
			pattern = element.getAttribute("pattern");
		}
		return new CrontabSchedule3(pattern);
	}
	...
	public final Element serialize(final Document doc) {
		...
		element.setAttribute("pattern", origPattern);
		return element;
	}


	public CrontabSchedule3(String pattern) throws DataParseException {
		read(pattern);
	}
	...
	public boolean parseToken(String token, boolean[] arrayBool, boolean bBeginInOne) throws DataParseException {
		int i;
		int index;
		int each = 1;
	...
	private boolean equalsCalendar(Calendar cal) {
		...
		bMinutes[cal.get(Calendar.MINUTE)] && bHours[cal.get(Calendar.HOUR_OF_DAY)] && bDaysOfWeek[cal.get(Calendar.DAY_OF_WEEK) - 1]
				&& bDaysOfMonth[cal.get(Calendar.DAY_OF_MONTH) - 1] && bMonths[cal.get(Calendar.MONTH)]);
