...
public class ProfileDetails extends org.eclipse.swt.widgets.Composite {
		...
		try {
			...
			}
			{
				...
				}
				{
					...
					{
						buttonUseFileFilter = new Button(simplyfiedOptionsGroup, SWT.CHECK | SWT.LEFT);
					...
					{
						buttonFileFilter = new Button(simplyfiedOptionsGroup, SWT.PUSH | SWT.CENTER);
	...
	public void apply() {
		...
		if (rbSimplyfiedRuleSet.getSelection()) {
			ruleSetDescriptor = new SimplyfiedRuleSetDescriptor(syncSubsButton.getSelection(), textIgnorePattern.getText(),
					textAcceptPattern.getText(), comboPatternsType.getText(), filter, buttonUseFileFilter.getSelection(), null);
		}
